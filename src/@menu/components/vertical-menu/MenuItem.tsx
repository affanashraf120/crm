...
const MenuItem: ForwardRefRenderFunction<HTMLLIElement, MenuItemProps> = (props, ref) => {
  ...
  useEffect(() => {
    ...
    if (href) {
      // Create a regex pattern to match the href and its subroutes
      const regexPattern = new RegExp(`^${href}(\/.*)?$`)

      // Check if the current url matches any of the children urls using regex
